...
Resume Studio is ify app' local-first resume editor.
...
- You already had a local Resume Studio document from an older ify app build:
...
- Changes do not appear in a generated PDF:
  Ready jobs that already use system-generated PDFs are auto-queued for regeneration after Resume Studio edits. If a job shows `PDF stale`, ify app is keeping the old PDF available while the new one is queued or regenerating.
