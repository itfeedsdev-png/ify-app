...
Reactive Resume integration powers ify app PDF generation.

ify app uses a selected RxResume base resume as the source of truth, then applies job-specific tailoring (summary, headline, skills, project visibility) before exporting a PDF.
...
Because RxResume uses structured JSON, ify app can safely apply LLM-driven updates to specific sections before generating PDFs.
...
3. Generate/store that password so ify app can use it for API login.

ify app cannot use OAuth-based RxResume logins for this integration.
