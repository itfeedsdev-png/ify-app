...
The Orchestrator is the primary jobs workspace in ify app.
...
Job rows and the detail header can also show a `Previously Applied` warning when ify app finds a high-confidence fuzzy match against one of your past applied or in-progress jobs using title and company, not just URL dedupe. To avoid flagging genuinely new openings, ify app only shows this warning when the matched historical application falls within 30 days of the current job's discovery date.
...
Date sorting follows the active date-filter context. If multiple date dimensions are enabled, ify app uses this priority:
...
If a job does not have the first selected timestamp, ify app falls back to the next available date in that order.
...
In the `ready` view, ify app can show prebuilt search links based on the current job's employer, title, and skills.
