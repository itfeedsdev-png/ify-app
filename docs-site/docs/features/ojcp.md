...
ify app exposes live [FreeHire](https://freehire.me/) search as an [Open Job Context Protocol](https://spec.ojcp.dev/0.1/) provider over MCP Streamable HTTP. Searches do not read or write the ify app jobs database.
...
The integration lets MCP-compatible agents discover current jobs without waiting for a ify app pipeline run.
...
1. Deploy ify app behind HTTPS.
2. Set the public base URL:

   ```bash
   IFYAPP_PUBLIC_BASE_URL=https://your-jobops-host
   ```
...
The running ify app server predates the MCP route, or the client is pointed at the frontend development server instead of the backend.

Restart the ify app server and use `http://localhost:3001/ojcp/mcp` for the default local backend.

### The manifest contains an HTTP or localhost endpoint

`IFYAPP_PUBLIC_BASE_URL` is missing or incorrect.

Set it to the externally reachable HTTPS origin and restart ify app.

### Search returns a provider error

FreeHire may be unavailable or may have exceeded its response deadline. Retry the search; the ify app pipeline and database do not act as a fallback.
