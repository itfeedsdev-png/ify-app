...
function resolveTracerReadinessBaseUrl(args: {
  ...
  const fromEnv = normalizeBaseUrl(process.env.IFYAPP_PUBLIC_BASE_URL ?? null);
...
export function resolveTracerPublicBaseUrl(args: {
  ...
  return normalizeBaseUrl(process.env.IFYAPP_PUBLIC_BASE_URL ?? null);
}
...
export async function getJobOpsPublicAvailability(
  ...
  if (!baseUrl) {
    response = makePublicAvailabilityResponse("unconfigured", {
      ...
      checkedAt,
      reason:
        "No public ify app base URL is configured. Set IFYAPP_PUBLIC_BASE_URL.",
    });
  } else {
