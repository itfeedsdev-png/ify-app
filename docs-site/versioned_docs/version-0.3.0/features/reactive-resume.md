...
description: Configure Reactive Resume as an optional import/export and PDF backend for ify app.
sidebar_position: 4
---

## What it is

Reactive Resume is now an optional integration in ify app.

The primary editing surface is **Design Resume**, which stores a local resume document inside ify app. Reactive Resume is used for:

- importing an existing resume into Design Resume
- exporting ify app resume JSON back out
...
Reactive Resume still matters because many users already maintain a strong resume there, and its JSON model is compatible with ify app. Importing that data into Design Resume avoids re-entry while reducing day-to-day dependence on the upstream service.
...
- Its JSON schema remains a stable interchange format for ify app.

For everyday editing inside ify app, use [Design Resume](./design-resume).
...
After import, ify app uses the local Design Resume for:
...
If you use the `rxresume` PDF renderer, ify app can still send the tailored JSON through Reactive Resume at export time.

### Project catalog

ify app reads projects from `sections.projects.items` in the local Design Resume document.
...
Before connecting Reactive Resume to ify app:
...
If you leave the URL blank in the dashboard, ify app uses `RXRESUME_URL` when it is set; if not set, it falls back to the public cloud default for the selected mode.

### Save-time validation

When you save Reactive Resume credentials or the shared URL in Settings:

1. ify app validates only the credential-bearing Reactive Resume fields for the selected mode.
...
3. Wait for ify app to create the local Design Resume document.

After that import, ify app reads resume context locally by default.
...
5. Normalize the tailored resume data into ify app' renderer document model.
...
ify app caches successful Reactive Resume resume fetches in memory for 5 minutes when upstream access is needed.
...
- `IFYAPP_PUBLIC_BASE_URL=https://your-host`
...
ify app can generate the final PDF in 2 ways:
...
- Project lists look stale:
  Re-import from Reactive Resume if you intentionally changed the upstream base resume and want those changes copied into ify app.
...
- Re-select/refresh the base resume in ify app and regenerate the PDF.
...
- Ensure `tectonic` is installed on the machine running ify app.
