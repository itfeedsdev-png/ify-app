...
- ify app uses the selected RxResume resume as the source of truth in both modes
...
- ify app ignores stale Gemini-style overrides under `openai`, and ignores stale OpenAI-style overrides under `gemini`, but you still need to save the current form selection for future runs.
