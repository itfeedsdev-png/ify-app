...
const PROVIDER_HINTS: Record<LlmProviderId, string> = {
  ...
  gemini_cli:
    "Gemini (CLI) runs the official Google Gemini CLI on this host using your OAuth session or CLI API key — no ify app LLM key.",
  claude_cli:
    "Claude (CLI) runs the official Claude Code CLI on this host using your subscription token or API key — no ify app LLM key.",
  codex:
