...
- Changes do not appear in a generated PDF:
  Ready jobs that already use system-generated PDFs are auto-queued for regeneration after Design Resume edits. If a job shows `PDF stale`, ify app is keeping the old PDF available while the new one is queued or regenerating.
