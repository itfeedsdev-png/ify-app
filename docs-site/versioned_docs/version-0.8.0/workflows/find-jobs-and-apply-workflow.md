...
This guide documents the main intended pre-application workflow in ify app.
...
3. If you wrote a resume outside ify app, use **Upload PDF** in the job detail view to attach that file to the application instead of using the generated version.
...
### 5) Mark jobs as applied in ify app

After submitting, return to ify app and mark the job as `applied`.
