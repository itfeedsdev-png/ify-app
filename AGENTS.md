...
This is an npm workspaces monorepo. Node 22.22.1 is required (managed via Volta).

Workspaces:
- `orchestrator/` — Main app (Express backend + React/Vite frontend). Entry: `src/server/index.ts`.
...
# 2. Database (SQLite, auto-created in ./data)
npm --workspace orchestrator run db:migrate

# 3. Dev servers (runs backend + frontend concurrently)
...
Biome enforces alias usage over relative parent imports. Use:
- `@infra/*` and `@shared/*` for shared server/shared modules
- `@client/*` for client modules
- `@server/*` for server modules
- `@career-boards/<name>` for career board entrypoints
