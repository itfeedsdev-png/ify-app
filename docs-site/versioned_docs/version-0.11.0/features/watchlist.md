...
Watching a company means ify app saves that company's careers source in your Watchlist and checks it again when you return, so you can review newly surfaced roles over time. It does not mean ify app watches every hiring channel from that company, sends alerts automatically, or applies on your behalf.

Each fetched row has one effective state:

- `new`: visible by default and ready to review
- `ignored`: hidden by default
- `moved_to_workspace`: already imported into the ify app workspace
...
ify app stores the selected careers source in your workspace and re-checks it when you reopen Watchlist. This gives you a repeatable review loop for newly surfaced roles from that company.
...
If you choose a company from the searchable company list, you are adding a catalog source. These shared entries come from ify app' maintained source catalog.
...
ify app uses that first successful check as the reference point for future `New since last check` badges. Before the baseline exists, there is nothing older to compare against.

### Ignored role

An ignored role is a Watchlist row hidden from your default review list for that user and workspace.

Ignoring a role does not delete it from the source website or from ify app storage. It only removes that row from the default visible list until you choose to show ignored rows again.
...
Rows already imported into ify app show **Already in workspace** and **Open workspace job**.

When you add a custom source URL, ify app asks that source adapter to derive a readable company label. If the adapter cannot derive a better label, the URL may still be the clearest identifier.

### Source URL rules

- Use the public careers URL, not an individual job posting URL.
- ify app validates the URL through the selected source adapter when you save sources.
...
ify app uses the source adapter's canonical source key plus the external job ID as the dedupe key, for example `workday:autodesk` and `26WD97952`. Open the existing workspace job from the Watchlist row.
