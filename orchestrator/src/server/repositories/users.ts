 ...
 * OAuth users are created without a real password (random bytes, never matchable).
 */
export async function findOrCreateOAuthUser(input: {
  ...
  // Derive a stable username: provider:accountId (lowercased)
  const username = normalizeUsername(
    `${input.provider}:${input.providerAccountId}`,
  );

  // Check if user already exists (idempotent)
  const existing = await getUserForLogin(username);
  if (existing) {
    const pub = await getUserById(existing.id);
    if (!pub) throw new Error("Failed to load OAuth user");
    return pub;
  }
  ...
  const unusableSalt = randomBytes(16).toString("base64url");
  ...
  const user = await getUserById(userId);
  if (!user) throw new Error("Failed to load created OAuth user");
  return user;
}
