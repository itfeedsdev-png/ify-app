...
- For `openai`, ify app defaults to `gpt-5.4-mini` when the model field is blank.
- For `codex`, ify app defaults to `gpt-5.4-mini` when the model field is blank.
- For `glm`, ify app defaults to `glm-5.1` and `https://api.z.ai/api/paas/v4` when the model/base URL fields are blank.
- For `gemini`, ify app defaults to `google/gemini-3-flash-preview` when the model field is blank.
