...
export async function importDesignResumeFromFile(
  ...
  // Parse-only mode for hackathon: skip LLM to minimize token usage
  if (process.env.IFYAPP_SKIP_LLM_RESUME_IMPORT === "true") {
    const parseOnlyText = await extractInitialDocumentText({
      provider,
      mediaType,
      decoded,
    }).catch(() => "");
    ...
    logger.info("Design resume file import completed (parse-only mode)", {
      requestId: requestId ?? null,
      provider,
      model: runtime.model,
      fileName,
