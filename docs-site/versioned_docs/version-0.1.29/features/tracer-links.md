...
When enabled for a job, ify app rewrites eligible outbound RxResume links to your ify app host, then redirects to the original destination after recording a click event.
...
ify app uses this URL when request host inference is not available.
...
- set `IFYAPP_PUBLIC_BASE_URL` for background flows
...
- ensure `IFYAPP_PUBLIC_BASE_URL` is set correctly
