...
describe.sequential("Jobs API routes", () => {
  ...
  it("blocks enabling tracer links when readiness check fails", async () => {
    ...
    const previousBaseUrl = process.env.IFYAPP_PUBLIC_BASE_URL;
    process.env.IFYAPP_PUBLIC_BASE_URL = "https://my-jobops.example.com";
    ...
    try {
      ...
      if (previousBaseUrl === undefined) {
        delete process.env.IFYAPP_PUBLIC_BASE_URL;
      } else {
        process.env.IFYAPP_PUBLIC_BASE_URL = previousBaseUrl;
      }
    }
  });
  ...
  it("prefers IFYAPP_PUBLIC_BASE_URL over forwarded headers for generate-pdf origin", async () => {
    const { createJob } = await import("@server/repositories/jobs");
    ...
    const previousBaseUrl = process.env.IFYAPP_PUBLIC_BASE_URL;
    process.env.IFYAPP_PUBLIC_BASE_URL = "https://canonical.jobops.example";
