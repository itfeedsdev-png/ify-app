...
describe("tracer-links service", () => {
  const originalEnv = process.env.IFYAPP_PUBLIC_BASE_URL;
  ...
  afterEach(() => {
    ...
    if (originalEnv === undefined) {
      ...
      process.env.IFYAPP_PUBLIC_BASE_URL = originalEnv;
    }
  });
  ...
  it("resolves public base url from request origin first, then env fallback", () => {
    process.env.IFYAPP_PUBLIC_BASE_URL = "https://fallback.example.com/";
  ...
  it("reports unconfigured readiness when no public base URL is available", async () => {
    ...
    expect(readiness.reason).toMatch(/no public ify app base url/i);
  });
