...
import { listSocialConnections } from "../api/social-media";
...
export const JobPage: React.FC = () => {
  ...
  const [shareDialogPlatform, setShareDialogPlatform] = React.useState<
  ...
  return (
    <main className="mx-auto max-w-[92rem] px-4 py-5">
        ...
        <div className={pageGridClass}>
          ...
          {activeMemoryView === "overview" && (
            <JobPageRightSidebar
              ...
              hasLinkedIn={Boolean(
                socialConnectionsQuery.data?.find(
              ...
              onShareToLinkedIn={() => setShareDialogPlatform("linkedin")}
              onShareToInstagram={() => setShareDialogPlatform("instagram")}
            />
          )}
        </div>
      )}
      ...
      <ShareSocialDialog
        open={shareDialogPlatform !== null}
        onOpenChange={(open) => {
          if (!open) setShareDialogPlatform(null);
        }}
        platform={shareDialogPlatform ?? "linkedin"}
        jobTitle={job?.title ?? ""}
