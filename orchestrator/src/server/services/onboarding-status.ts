...
export async function validateRxresume(options?: {
  ...
  if (result.status === 0 || result.status >= 500) {
    return {
      valid: false,
      status: result.status,
      message: `ify app could not verify Reactive Resume because the instance at ${resolvedBaseUrl} is unavailable right now.`,
    };
  }
...
function getModelValidationMessage(args: {
  ...
  const baseMessage =
    ...
    `ify app could not verify ${args.providerLabel}. Check the connection and try again.`;

  if (args.provider === "ollama" && isUnavailable(args.validation)) {
    return `${baseMessage} Local Ollama models can be slow or unavailable on smaller hardware. Try a smaller or faster model, make sure Ollama is reachable from the ify app container, add more CPU/GPU/RAM, or switch provider from the model step.`;
  }
...
async function buildResumeRequirement(): Promise<OnboardingRequirement> {
    ...
    return buildRequirement({
      ...
      message:
        confirmedSource === source
          ? "You confirmed the resume ify app should use for matching and applications."
          : "Check the parsed resume, then confirm that ify app loaded the correct document.",
  ...
  if (!resumeId) {
    ...
    if (rxresumeValidation.valid) {
      return buildRequirement({
        ...
        message:
          "Reactive Resume is connected. Select the resume ify app should use for matching, fit assessment, and applications.",
    ...
    return buildRequirement({
      ...
      message:
        "Upload a resume file, or connect Reactive Resume and choose a template. This gives ify app the baseline it needs for matching, fit assessment, and better application workflows.",
  ...
  if (validation.valid) {
    ...
    return buildRequirement({
      ...
      message:
        confirmedSource === source
          ? "You confirmed the Reactive Resume template ify app should use."
  ...
  return buildRequirement({
    ...
    message:
      validation.message ||
      "ify app could not verify the selected resume. Recheck the source or choose another template.",
    primaryAction:
      status === "checking_unavailable"
        ? "recheck"
        : authIssue
...
async function buildHostedResumeRequirement(): Promise<OnboardingRequirement> {
  ...
  return buildRequirement({
    ...
    message:
      "Upload your existing resume as a PDF or DOCX. ify app will use it as the baseline for matching, fit assessment, search terms, and application workflows.",
    primaryAction: "upload_resume",
...
export async function getOnboardingStatus(): Promise<OnboardingStatusResponse> {
  ...
  const skipLlmOnboarding =
    process.env.IFYAPP_SKIP_LLM_ONBOARDING?.trim().toLowerCase() === "true";
  const userEditableLlmSettings =
    !skipLlmOnboarding && appStatus.capabilities.userEditableLlmSettings;
  ...
  const profileRequirement = buildRequirement({
    ...
    message:
      profileCompleted === "1"
        ...
        : "Add your preferred locations and workplace style so ify app can prepare sensible run defaults and sponsor-focused sources.",
