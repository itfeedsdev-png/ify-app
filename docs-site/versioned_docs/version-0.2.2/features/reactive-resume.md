...
Reactive Resume provides the structured base resume that ify app uses for PDF generation.

ify app uses a selected RxResume base resume as the source of truth, applies job-specific tailoring (summary, headline, skills, project visibility), then renders the final PDF using the renderer selected in Settings.
...
### Why ify app uses RxResume (instead of building a new editor)
...
- It has a JSON-native model (import/export), which is critical for ify app automation.

Because RxResume uses structured JSON, ify app can safely apply LLM-driven updates to specific sections before rendering PDFs locally.
...
ify app reads projects from `sections.projects.items` in the selected RxResume resume.
...
ify app caches successful Reactive Resume resume fetches in memory for 5 minutes.
