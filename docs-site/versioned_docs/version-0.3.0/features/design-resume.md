...
id: design-resume
title: Design Resume
description: Edit the local resume document that ify app uses for tailoring, scoring, and PDF generation.
sidebar_position: 4
---

## What it is

Design Resume is ify app' local-first resume editor.

It stores an exact Reactive Resume v5 document inside ify app. ify app does not convert that document into a separate internal resume format. ify app uses this local RR v5 document as the primary source of truth for:
...
Depending on Reactive Resume for every profile lookup, project read, and PDF flow makes ify app more fragile than it needs to be.

Design Resume reduces that dependency by letting you:

- upload a PDF or DOCX resume directly into ify app
- import from Reactive Resume once
- keep editing locally inside ify app
...
3. Wait for ify app to create or replace the local Design Resume.
...
- There is no OCR or local text-extraction fallback. If the configured model cannot accept attached files, ify app returns an error instead.
...
- File import says the model cannot handle the file directly:
  Switch to a model with native document upload support, then try the import again. ify app does not fall back to OCR or local text extraction.
- You already had a local Design Resume from an older ify app build:
