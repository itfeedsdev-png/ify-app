...
description: Deploy ify app with Docker Compose and configure onboarding integrations.
sidebar_position: 1
---

The easiest way to run ify app is via Docker Compose. The app is self-configuring and guides you through setup on first launch.
...
ify app passes this token to the Docker build as a BuildKit secret only for the
...
2. **Search preferences**: Save your country, optional preferred cities, workplace style, and whether you need visa sponsorship. ify app applies these values as defaults for future runs and sponsor-aware features.
...
4. **Resume review**: Upload a PDF/DOCX/Reactive Resume JSON, or connect Reactive Resume. ify app shows the parsed resume and requires confirmation of the current document before setup completes.

Search terms are not part of onboarding. Choose or edit them when starting your first run.

Settings and user accounts are saved to the local database. If you are upgrading an older single-user install, ify app migrates existing rows into one default private workspace. When `BASIC_AUTH_USER` and `BASIC_AUTH_PASSWORD` are present during that migration, they seed the first system admin account; otherwise onboarding requires first-run account setup before private APIs are usable.
...
ify app defaults to local/self-hosted mode when hosted flags are unset. Hosted mode is an env-gated deployment mode for running one configured tenant with multiple users in that tenant.
...
`IFYAPP_HOSTED_TENANT_ID` is required in hosted mode. ify app fails startup if `IFYAPP_APP_MODE=hosted` is set without a hosted tenant ID. The tenant must already exist in the database before hosted signup can create users for it; hosted signup does not create tenants.

Optional hosted capabilities default to disabled:

```bash
IFYAPP_HOSTED_SIGNUPS_ENABLED=false
IFYAPP_HOSTED_PLATFORM_LLM_ENABLED=false
IFYAPP_HOSTED_QUOTAS_ENABLED=false
```

These flags only affect hosted mode. Leaving `IFYAPP_APP_MODE` unset keeps the current first-run setup, private-workspace user creation, settings, and local behavior unchanged.

When hosted mode is active, first-run setup is disabled. Hosted users must be created through hosted signup when `IFYAPP_HOSTED_SIGNUPS_ENABLED=true`, or by a later tenant-owner/admin user-management flow. Hosted signup appears on `/sign-in` as a **Create account** tab only when hosted mode and hosted signups are both enabled.
...
- `RXRESUME_API_KEY=...` (or configure `rxresumeApiKey` in ify app Settings)



## Local PDF rendering

ify app supports 3 PDF renderers:
