...
2. ify app detects the country at that point and shows it below the map.
...
After you select or adjust an area, ify app previews the named cities and towns inside the circle. The location count in **Your search** updates after this lookup and the same result is reused when the search starts.
...
- Sources that accept location strings receive up to 25 nearby OpenStreetMap city or town names, ranked by proximity and population. If the Overpass service is unavailable, ify app uses the locality nearest the selected centre instead of failing the whole run.
...
When you place or move the centre, ify app sends it to OpenStreetMap Nominatim to detect the country. When you run a radius search, ify app sends the centre and radius to the OpenStreetMap Overpass service to resolve nearby place names. OpenStreetMap tile servers also receive normal map-tile requests while the map is visible. If Hiring Cafe is selected, its search request receives the same centre and radius. Coordinates are stored in tenant-scoped settings and are not written to application logs.

#### Country and source compatibility

- The detected map country—or the country selected in Manual cities mode—affects which sources are available, but the country list is owned by ify app rather than by any single extractor.
