...
The primary editing surface is **Resume Studio**, which stores a local resume document inside ify app. Reactive Resume is used for:
...
Reactive Resume still matters because many users already maintain a strong resume there, and its JSON model is compatible with ify app. Importing that data into Resume Studio avoids re-entry while reducing day-to-day dependence on the upstream service.
...
For everyday editing inside ify app, use [Resume Studio](./design-resume).
...
After import, ify app uses the local Resume Studio document for:
...
ify app reads projects from `sections.projects.items` in the local Resume Studio document.
...
If you leave the URL blank in the dashboard, ify app uses `RXRESUME_URL` when it is set; if not set, it falls back to the public cloud default (https://rxresu.me).

### Save-time validation

When you save Reactive Resume credentials or the shared URL in Settings:

1. ify app validates your API key.
...
3. Wait for ify app to create the local Resume Studio document.
...
ify app can generate the final PDF in 3 ways:
