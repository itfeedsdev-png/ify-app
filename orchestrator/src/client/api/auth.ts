...
export async function getOAuthProviders(): Promise<{
  ...
  return fetchApi<{ google: boolean; github: boolean }>(
    "/auth/oauth/providers",
  );
}

/**
 * Redirect the browser to start the OAuth flow for the given provider.
 * After the provider redirects back to the callback URL, the server will
 * redirect to /sign-in?oauth_token=<JWT>&oauth_expires_in=<seconds>.
 */
